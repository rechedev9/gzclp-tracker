...
export function NotFound(): React.ReactNode {
  return (
    <div className="min-h-dvh flex flex-col items-center justify-center bg-[var(--bg-body)] px-6 text-center relative overflow-hidden">
      {/* Ambient glow */}
      <div
      ...
      />
      <h1
        className="font-display text-7xl sm:text-8xl text-[var(--text-header)] mb-3 relative"
        style={{ textShadow: '0 0 40px rgba(240, 192, 64, 0.2)' }}
      >
        404
      </h1>
      <p className="text-sm text-[var(--text-muted)] mb-8 max-w-xs relative">
        Esta página no existe. Puede que haya sido movida, o escribiste la URL incorrectamente.
      </p>
      <Link
        to="/"
        className="px-6 py-3 text-xs font-bold border-2 border-[var(--btn-border)] bg-[var(--btn-hover-bg)] text-[var(--btn-hover-text)] hover:opacity-90 transition-all active:scale-[0.97] relative"
      >
        Ir al Inicio
      </Link>
    </div>
